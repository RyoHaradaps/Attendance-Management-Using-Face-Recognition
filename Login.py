...
class Login:
    def __init__(self,root):
        ...
        self.bg=ImageTk.PhotoImage(file=r"C:\Users\surya\.vscode\Face recognition IOC\Interface_images\red-and-blue.jpeg")
        
        lb1_bg=Label(self.root,image=self.bg)
        lb1_bg.place(x=0,y=0, relwidth=1,relheight=1)

        frame1= Frame(self.root,bg="#002B53")
        frame1.place(x=600,y=170,width=340,height=450)

        img1=Image.open(r"C:\Users\surya\.vscode\Face recognition IOC\Images_GUI\log1.png")
        img1=img1.resize((100,100),Image.Resampling.LANCZOS)
        self.photoimage1=ImageTk.PhotoImage(img1)
        lb1img1 = Label(image=self.photoimage1,bg="#002B53")
        lb1img1.place(x=725,y=180, width=100,height=100)
    ...
    def login(self):
        if (self.txtuser.get()=="" or self.txtpwd.get()==""):
            ...
            mycursor.execute("select * from reglog where email=%s and pwd=%s",(
                self.txtuser.get(),
                self.txtpwd.get()
            ))
...
#=======================Reset Passowrd Function=============================
    def reset_pass(self):
        if self.var_ssq.get()=="Select":
            ...
            conn = mysql.connector.connect(username='root', password='4125',host='localhost',database='face_recog')
            mycursor = conn.cursor()
            query=("select * from reglog where email=%s and ss_que=%s and s_ans=%s")
            ...
            if row==None:
                ...
                query=("update reglog set pwd=%s where email=%s")
...
# =====================Forget window=========================================
    def forget_pwd(self):
        if self.txtuser.get()=="":
            ...
            conn = mysql.connector.connect(username='root', password='4125',host='localhost',database='face_recogn')
            mycursor = conn.cursor()
